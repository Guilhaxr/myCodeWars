...
//  and your goal is to work out the minimum amount of bombs you need to drop in order to fully extinguish
//   the fire (the fire department has budgeting concerns and you can't just be dropping tons of bombs,
...
function waterbombs(fire, w) {
    let fireArr =fire.split('Y');
    let counter =0;

    const calculation = fireArr.map((e) => Math.ceil(e.length / w));
    
    const finalCalculation = calculation.reduce(( a, b) => a + b)
